...
if os.path.exists(BCW_file_path):
    ...
    for depth in max_depths:
        base_model = DecisionTreeClassifier(max_depth=depth, random_state=42)
        model = AdaBoostClassifier(estimator=base_model, n_estimators=n_estimators, random_state=42, algorithm='SAMME')
        scores = cross_val_score(model, X_cancer, y_cancer, cv=nFold)
        print("Do chinh xac cua mo hinh voi depth = %d va %d-fold: %.3f" % (depth, nFold, np.mean(scores)))
