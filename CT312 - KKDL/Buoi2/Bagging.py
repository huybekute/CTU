...
if os.path.exists(BCW_file_path):
    ...
    model = BaggingClassifier(estimator=base_model, n_estimators=n_estimators, random_state=42)
    scores = cross_val_score(model, X_cancer, y_cancer, cv=nFold)
    print("So luong cay: %d | Do chinh xac: %.3f" % (n_estimators, np.mean(scores)))
